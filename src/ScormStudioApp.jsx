...
function generateCourseHtml(course) {
    ...
    for (let si = 0; si < (mod.slides||[]).length; si++) {
      ...
      if (slide.type === 'title') {
        ...
        const optHtml = (slide.options||[]).map((o,oi) =>
          `<button class="scenario-option" onclick="handleScenario(this,${o.correct},'${fid}')">${esc(o.text)}</button>`
        ).join('');
        const fbHtml = (slide.options||[]).map(o => esc(o.feedback)).join(' | ');
  ...
  const courseJson = JSON.stringify({ ...course, modules: moduleMap });
